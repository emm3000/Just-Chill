...
This module is the repo's composition-test pilot; `:feature:onboarding` (#240) and `:feature:category` (#272) copied the harness. Robolectric and `androidx.compose.ui:ui-test-junit4` are `testImplementation` in those two modules only, with `ui-test-manifest` on `debugImplementation` for the host activity, and `testOptions.unitTests.isIncludeAndroidResources` is enabled in each module's own build file rather than in `AndroidLibraryConventionPlugin`, so no other library module pays for it.
