...
This module is the repo's composition-test pilot; `:feature:onboarding` (#240), `:feature:category` (#272) and `:feature:report` (#271) copied the harness. Robolectric and `androidx.compose.ui:ui-test-junit4` are `testImplementation` only in the modules that carry the harness and nowhere else, with `ui-test-manifest` on `debugImplementation` for the host activity, and `testOptions.unitTests.isIncludeAndroidResources` is enabled in each module's own build file rather than in `AndroidLibraryConventionPlugin`, so no other library module pays for it.
