...
`ProfileRoute` is a `BottomBarRoute`, the `Más` tab (ADR 022), which the pad's menu icon also selects; `PrivacyPolicyRoute` is another, pushed from this screen, the one door into `privacy/`. Both are in `profileRoutes`, which `RouteSerializationTest` concatenates. Transactions, the report, accounts, categories, recurring movements, loans, the manifesto and sign-in all leave this feature, so each arrives as an `(AppNavigator) -> Unit` callback supplied by `AppEntryGraph.kt`, never as a route value. The Movimientos, Reporte and Cuentas rows take `AppNavigator.selectTab`; every other destination row takes `pushToTop`, never `push`, which silently does nothing on a buried route. `pushToTop` cannot strand the user here, because the list owns index 0 and this tab index 1, so the deepest a row can truncate to is `[list, Más]`. `rememberPlatformHostActions` stays in `:androidApp`'s `shell/`: the SAF launchers must be registered at the nav host root, or a picker result arriving after its entry left composition is dropped.
