...
- `severity()` and `toMetaText()` stay out of composables (`ProfileViewModelBackupRowTest` pins them). `:androidApp`'s `disclosureIsPending` and `resolveBackupRow` answer the same question and must agree. The first one feeds `BackupDisclosureSignal`, whose reader is `AppNavHost`'s bar: a `warning` dot on the Más slot while a signed-in account has not acknowledged the disclosure.
