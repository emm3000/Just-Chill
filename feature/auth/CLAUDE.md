...
- `authEntries` takes only `NavHostBindings`: nothing navigates out of auth except `nav.pop()`. The door in is Profile's `onSignInClick: (AppNavigator) -> Unit`, supplied from `AppNavHost`, since `:ui-android` cannot import this module.
