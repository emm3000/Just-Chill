...
- A category filter, or an amount range, turns the list into a cross-month search, so the month selector steps aside; in month mode `SeeTransactionsUiState.isEyebrowVisible` shows the row while the ledger count is still unknown, and hides it once the count is known to be zero (`EmptyLedger`), because a spend eyebrow naming nothing would say nothing (#452).
