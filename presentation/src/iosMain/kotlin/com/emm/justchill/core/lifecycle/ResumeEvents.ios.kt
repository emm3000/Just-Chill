 ...
 *
 * Wired into `BackupOrchestrator` (`core/backup/`) as its `resumeEvents` flow (the platform-injected
 * on-resume signal), so the on-resume backup trigger reaches Android parity. Both platforms resolve
 * through the single common `resumeEvents()` expect declaration — this is its iOS actual.
 *
