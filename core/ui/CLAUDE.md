...
- `JcTopBar` pads its own edges off that giveback: the start is `s4 - edgeGiveback(s6)`, so `BackBtn`'s 24dp chevron lands on the `s4` column, and the end is `s4 - edgeGiveback(rightArtwork)`, the artwork the caller paints in `right`. It defaults to `s12`, the full slot an `IconBtn`, a `HeaderAction` or a pill fills with a bordered box; a caller whose artwork is smaller than its touch target passes that size instead, as Reporte does with `TopBarTileSize` for its 44dp share tile. The `s2` gap sits on both sides of the title, so an overflowing one never ends flush against a slot.
