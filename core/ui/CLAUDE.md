...
- `sheets/` holds `AccountPickerSheet`, `CategoryPickerSheet`, `DatePickerSheet` and `MonthPickerSheet`, shared by the transaction, recurring, loan and report forms. A sheet used by one feature stays in that feature's own module — `NoteSheet` is the example.
