...
`DefaultBackupRepository` is the one deliberate exception to the `Default{Entity}Repository` → `{Entity}LocalDataSource` shape: a snapshot and a restore each have to be a single transaction spanning every table, and per-entity data sources cannot share one, so it drives `JustChillDatabase` and its `*Queries` directly.
...
- Host tests (JUnit4 + MockK) in `core/database/src/test/kotlin/`: mappers, enum parsing, backup, plus plain `kotlin.test` suites. `./gradlew :core:database:testDebugUnitTest`.
