...
- `SnackbarHostState` lives in the root `Scaffold` of `AppNavHost.kt`. `compose_stability.conf` declares `:presentation`'s state classes and `:domain` values stable; if a list stutters, read the compiler metrics first.
