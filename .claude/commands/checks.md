...
Group findings by module (`:core:domain`, `:core:database`, `:presentation`, `:ui-android`, `:androidApp`, `:build-logic`). For each violation include `file:line` and the check or test name. Test tasks go `UP-TO-DATE` across sessions; add `--rerun` per task when a real run is needed.
