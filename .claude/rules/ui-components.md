...
- **The press is acknowledged at finger-down**, from the same `MutableInteractionSource` the `clickable` takes. A shaped control — pill, chip, tile, circle, button — draws `ripple()` clipped to its shape (`:core:ui`'s `core/ui/atoms/IconBtn.kt`); a full-width row on `bg` swaps its ground to `surface1` through `collectIsPressedAsState()` (`:feature:profile`'s `ProfileRow.kt`, `ProfileRowWithTrailing`, and `:feature:category`'s `CategoriesScreen.kt`). `indication = null` is allowed only where that same source is read in the same composable. One exemption: a selection control whose selected fill repaints under the finger, `core/ui/atoms/Segmented.kt`'s `SegmentCell`, where the repaint is the acknowledgment. Fifteen seconds a movement means the finger lifts trusting the tap landed; a control that answers only at UP gets tapped twice, and on a navigating row the second tap lands on the next screen.
- **A navigable row ends with `Icons.Outlined.ChevronRight`** at `spacing.s4`, `textTertiary`, `contentDescription = null`, after the amount column when there is one — navigable meaning its tap pushes a screen that holds more rows. A row that opens its own editor, a sheet or a dialog carries none. Never a bare trailing edge on a navigable row, never a chevron on a sheet or dialog row, never `onClickLabel` as the only hint: it reaches TalkBack and nobody else. The PRD's "Es descubrible" row — a level nobody can see is a level nobody uses. The canonical glyph is `ChevronTrailing`, today `internal` in `:feature:profile` and expected as a `:core:ui` atom.
- **A disabled or loading control keeps its `clickable`** with `enabled = false` and `role = Role.Button`, so it stays in the accessibility tree and a screen reader announces a disabled button (`core/ui/atoms/OutlinedCta.kt`, `core/ui/atoms/EmmDialog.kt`); its ground is `surface1`, its label `textTertiary`, and it never leaves layout (`core/ui/atoms/IconBtn.kt`). Never `.then(if (enabled) Modifier.clickable(...) else Modifier)`, never `textDisabled` on the label that names the blocked action — that token is for the meta text beside it — never a control hidden because it is disabled. The PRD's Accesibilidad row: a disabled primary action is the app saying what is missing, and read as plain text it says nothing.
- **A touch box larger than its artwork** takes `clickable(interactionSource, indication = null)`, and the painted child takes `.clip(shape)` then `.indication(interactionSource, ripple())`, before its padding (`:core:ui`'s `core/ui/sheets/DatePickerSheet.kt`, `ShortcutPill` and `DayCell`). The order is load-bearing: `clip` before `indication` or the ripple squares, `indication` before `padding` or the label sits outside it. `ripple(bounded = false)` is never used. After the 48dp work this is the dominant shape in the codebase, and a ripple on the box reveals the box — a rectangle flashing in a grid of circles.
