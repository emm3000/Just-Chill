...
- The Crashlytics mapping upload runs only with `-Pjustchill.crashlyticsMappingUpload=true`, which `uploadApk` passes on `trunk` and `uploadRelease` on a release tag. PRs, Dependabot and local builds never upload.
