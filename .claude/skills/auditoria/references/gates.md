...
| Architecture / module | `CLAUDE.md` `## Architecture`, then that module's own `CLAUDE.md` (`androidApp/`, every `core/*/` and every `feature/*/` ships one; `build-logic/` and `supabase/` have none) and the `.claude/rules/` file scoped to it | 20 |
| Sync | `core/database/CLAUDE.md` `## Backup`, `androidApp/CLAUDE.md` `## Backup` and `docs/adr/009-backup-is-a-snapshot-not-row-replication.md` BEFORE any source file | 20 |
| Supabase / SQLDelight migrations | `.claude/rules/sqldelight.md` and `core/database/CLAUDE.md` `## Persistence`, then `supabase/migrations/` against the SQLDelight schema. Flag any drift — this is the class that broke production for two months (commit `72a9b03`) | 15 |
