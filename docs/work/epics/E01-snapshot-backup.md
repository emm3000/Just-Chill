...
- Backup row state resolution and copy live in `:presentation` (`BackupRowUi.severity()`, `toMetaText()`), never in a composable — that is the `:presentation`/`:ui-android` split, and only `:presentation`'s host tests can pin it (`:ui-android` has no Compose UI test harness).
