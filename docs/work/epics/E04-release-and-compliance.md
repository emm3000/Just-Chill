...
- **E04-01 (#87) and E04-02 (#88) end in the Play Console**, so no amount of repo work closes them —
  the last step is a person in a web form. E04-05 (#89) is not blocked on that; it waits on art nobody
  has authored.
- **The restore drill is required by
  [ADR 009](../../adr/009-backup-is-a-snapshot-not-row-replication.md) Decision 4 and is enforced
  only by its line on [`RELEASE_CHECKLIST.md`](../../RELEASE_CHECKLIST.md).** Delete that line and
  nothing in the repo asks for the drill again.
