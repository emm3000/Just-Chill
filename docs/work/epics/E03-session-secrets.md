...
The Supabase refresh token is the account. It is the one value in this app worth stealing, and the
store a mobile app reaches for by default — `SharedPreferences` — keeps it in the clear.

## Constraints

- A persisted credential lives in the platform's secret store — the Android Keystore — or it does
  not persist. `SharedPreferences` is not a secret store. It is the easy path, which is why this
  epic exists.
...
- `SessionManager` is bound **once**, in `AndroidPlatformModule.kt`. A credential added later
  inherits whatever backing store that binding already has, so the binding is the thing to review —
  never the call site.
...
- **Moving a credential off a cleartext store has to be eager.** `SessionManager` hangs off a lazy
  ...
  `EmmApp` sweeps at launch instead.
...
- **A sweep on the launch path never throws.** Android's `sweepLegacySession` is `runCatching` so a
  Keystore that cannot serve a key does not kill `onCreate`.
