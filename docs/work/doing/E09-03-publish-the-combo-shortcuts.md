...
- A core-side source returns shortcut-ready combos — ids, type and both labels as plain strings, no
  ...
  **Spend only** — income is a planned monthly event nobody reaches for a launcher shortcut to log.
- The publisher lives where `MainActivity` is reachable, builds an explicit `Intent` with
  `ACTION_ADD_TRANSACTION`, the `EXTRA_*` constants, and the dev flavor's suffixed application id.
