...
- The bottom bar (`shell/AppBottomBar.kt`) has five slots, no more; a new destination swaps a tab out. `AppNavHost`'s `Scaffold` draws it only while the top of the stack is a `BottomBarRoute` (the four tab roots) and the keyboard is closed, so every pushed screen hides it and the content keeps the IME inset. A tab tap is `AppNavigator.selectTab`, which keeps the root entry and swaps what sits above it; the add key pushes `AddTransactionRoute()`.
