...
`com.android.application` on AGP 9's built-in Kotlin, root package `com.emm.justchill`, `minSdk = 28`, `compileSdk = 37`. Depends on the eight `:feature:*` modules, on `:core:ui` and `:core:domain`, and on `:core:database` and `:core:backup`, which only this module sees: `core/di/DataModule.kt` binds every repository and `SnapshotStore`, and `AndroidPlatformModule` constructs the SQLDelight driver itself.

## The shell

- `shell/AppNavHost.kt` owns the back stack, the root `Scaffold` with its `SnackbarHostState`, the SAF launchers and the result channels, which it hands to `shell/AppEntryGraph.kt` as one `HostResultChannels`; that file holds the eight features' `*Entries` calls and every cross-feature route the host supplies. `rememberPlatformHostActions` (`shell/PlatformHostActions.kt`) builds the export and import SAF launchers and must be called at that root, never inside an `entry<...> { }` body, or a picker result arriving after its entry left composition is dropped. `AppNavigator` and `NavHostBindings` are `:core:ui`'s; `RouteSerializationTest` concatenates one route registry per feature.
- `HOME_ROUTE` (`shell/AppEntryGraph.kt`) is the Movimientos list, `SeeTransactionRoute`, and it is the only thing that sits at index 0 (ADR 022): the app opens on it, the manifesto's `replaceAll` lands on it, and the system back gesture exits the app from it.
- The bottom bar (`shell/AppBottomBar.kt`) has five slots, no more; a new destination swaps a tab out. `AppNavHost`'s `Scaffold` draws it only while the top of the stack is a `BottomBarRoute` (the four tab roots) and the keyboard is closed, so every pushed screen hides it and the content keeps the IME inset. A tab tap is `AppNavigator.selectTab`, which keeps the root entry and swaps what sits above it; the add key pushes `AddTransactionRoute()`. The bar never touches Koin: `AppNavHost` collects `BackupDisclosureSignal.isPending` and passes `profileNeedsAttention`, which draws a `warning` dot on the Más slot and names it `Más, requiere tu atención`.
...
- `BackupAvailability` (`:core:domain`) is `FlavorBackupAvailability`, bound in `core/di/BackupModule.kt` over the `SNAPSHOT_BACKUP_ENABLED` `buildConfigField` each flavor declares in `build.gradle.kts`: `dev` true, `prod` false. Readers here, both skipping when it is false: `bootstrapAppGraph`, which skips `start()` and leaves `BackupOrchestrator` bound and lazily resolvable, and `BackupDisclosureSignal`, whose `isPending` gates to `flowOf(false)` because with the orchestrator stopped `canUploadToDestination` stays false for every account and an ungated signal would announce a gate that cannot run. Its reader is `AppNavHost`'s bar, through a `koinInject` `AppGraphKoinTest` cannot see; `singleOf` in `backupModule` is what puts it in the sweep.
