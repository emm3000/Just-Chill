...
- `:core:testing` — the JVM test-fixture module: `MainDispatcherRule` and `FakeTodayFlow`, on `:core:domain` only, wired into feature modules, `:core:ui` and `:androidApp` as `testImplementation`.
