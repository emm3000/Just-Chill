...
- `:feature:{account, category, loan, report}` populated (ADR 015 waves 6-7): ViewModels, screens, routes, entries, a Koin module with ViewModels only, tests. `:feature:{transaction, recurring, profile, auth, onboarding}` wait for wave 8 (#123-#127).
- `:presentation` — the not-yet-extracted ViewModels (transaction, recurring, profile, auth, onboarding) with their `UiState` / `Intent` / `Effect`, the Koin modules, the feature copy. Compose-free, and it re-exports `:core:ui` as `api`.
- `:ui-android` — Compose screens and nav entries for the not-yet-extracted features, with `rememberPlatformHostActions`. Same Kotlin packages as `:presentation` on purpose.
